...
void test_tracker_kalman_gain(void){
    //final K = {0.95238,0.23810}
    printf("testing Kalman_gain calculation in batch with the self defined tracker object:\n");
    ...
    std::cout<<"No. of tracks tested:"<<N<<", measurement vector size: "<<m
                <<", state vector size: "<<n<<"\n";
    ...
    printf("output kalman gain K=P*H^T*S^(-1),  S=H*P*H^T+R:\n");
    std::cout<<"final K = {0.95238,0.23810}, checking final result\n";
    for (int i=0;i<N;i++){
    ...
    printf("All K values match expected. No error.");
